...
package xyz.inphinity.sun.security.mscapi;

import java.security.*;
import java.util.HashMap;
import java.util.List;

import static xyz.inphinity.sun.security.util.SecurityConstants.PROVIDER_VER;
import static xyz.inphinity.sun.security.util.SecurityProviderConstants.getAliases;
...
public final class SunMSCAPIAlt extends Provider {

    private static final long serialVersionUID = 8622598936488630849L; //TODO

    private static final String INFO = "Sun's Microsoft Crypto API provider - modified by Inphinity";

    static {
        @SuppressWarnings("removal")
        var dummy = AccessController.doPrivileged(new PrivilegedAction<Void>() {
            public Void run() {
                System.loadLibrary("mscrypto-alt");
                return null;
            }
        });
    }
    ...
    @SuppressWarnings("removal")
    public SunMSCAPIAlt() {
        super("SunMSCAPI-alt", PROVIDER_VER, INFO);
        ...
        AccessController.doPrivileged(new PrivilegedAction<Void>() {
            public Void run() {
                ...
                putService(new ProviderService(p, "SecureRandom",
                           "Windows-PRNG", "xyz.inphinity.sun.security.mscapi.PRNG",
                           null, srattrs));
                ...
                putService(new ProviderService(p, "KeyStore",
                           "Windows-MY", "xyz.inphinity.sun.security.mscapi.CKeyStore$MY"));
                putService(new ProviderService(p, "KeyStore",
                            "Windows-MY-CURRENTUSER", "xyz.inphinity.sun.security.mscapi.CKeyStore$MY"));
                putService(new ProviderService(p, "KeyStore",
                           "Windows-ROOT", "xyz.inphinity.sun.security.mscapi.CKeyStore$ROOT"));
                putService(new ProviderService(p, "KeyStore",
                            "Windows-ROOT-CURRENTUSER", "xyz.inphinity.sun.security.mscapi.CKeyStore$ROOT"));
                putService(new ProviderService(p, "KeyStore",
                            "Windows-MY-LOCALMACHINE", "xyz.inphinity.sun.security.mscapi.CKeyStore$MYLocalMachine"));
                putService(new ProviderService(p, "KeyStore",
                            "Windows-ROOT-LOCALMACHINE", "xyz.inphinity.sun.security.mscapi.CKeyStore$ROOTLocalMachine"));

                /*
                 * Signature engines
                 */
                HashMap<String, String> attrs = new HashMap<>(1);
                attrs.put("SupportedKeyClasses", "xyz.inphinity.sun.security.mscapi.CKey");
                ...
                putService(new ProviderService(p, "Signature",
                           "NONEwithRSA", "xyz.inphinity.sun.security.mscapi.CSignature$NONEwithRSA",
                           null, attrs));
                putService(new ProviderService(p, "Signature",
                           "SHA1withRSA", "xyz.inphinity.sun.security.mscapi.CSignature$SHA1withRSA",
                           null, attrs));
                putService(new ProviderServiceA(p, "Signature",
                           "SHA256withRSA",
                           "xyz.inphinity.sun.security.mscapi.CSignature$SHA256withRSA",
                           attrs));
                putService(new ProviderServiceA(p, "Signature",
                           "SHA384withRSA",
                           "xyz.inphinity.sun.security.mscapi.CSignature$SHA384withRSA",
                           attrs));
                putService(new ProviderServiceA(p, "Signature",
                           "SHA512withRSA",
                           "xyz.inphinity.sun.security.mscapi.CSignature$SHA512withRSA",
                           attrs));
                putService(new ProviderServiceA(p, "Signature",
                           "RSASSA-PSS", "xyz.inphinity.sun.security.mscapi.CSignature$PSS",
                           attrs));
                putService(new ProviderService(p, "Signature",
                           "MD5withRSA", "xyz.inphinity.sun.security.mscapi.CSignature$MD5withRSA",
                           null, attrs));
                putService(new ProviderService(p, "Signature",
                           "MD2withRSA", "xyz.inphinity.sun.security.mscapi.CSignature$MD2withRSA",
                           null, attrs));
                putService(new ProviderServiceA(p, "Signature",
                           "SHA1withECDSA",
                           "xyz.inphinity.sun.security.mscapi.CSignature$SHA1withECDSA",
                           attrs));
                putService(new ProviderServiceA(p, "Signature",
                           "SHA224withECDSA",
                           "xyz.inphinity.sun.security.mscapi.CSignature$SHA224withECDSA",
                           attrs));
                putService(new ProviderServiceA(p, "Signature",
                           "SHA256withECDSA",
                           "xyz.inphinity.sun.security.mscapi.CSignature$SHA256withECDSA",
                           attrs));
                putService(new ProviderServiceA(p, "Signature",
                           "SHA384withECDSA",
                           "xyz.inphinity.sun.security.mscapi.CSignature$SHA384withECDSA",
                           attrs));
                putService(new ProviderServiceA(p, "Signature",
                           "SHA512withECDSA",
                           "xyz.inphinity.sun.security.mscapi.CSignature$SHA512withECDSA",
                           attrs));
                /*
                ...
                putService(new ProviderService(p, "KeyPairGenerator",
                           "RSA", "xyz.inphinity.sun.security.mscapi.CKeyPairGenerator$RSA",
                           null, attrs));
                ...
                putService(new ProviderService(p, "Cipher",
                           "RSA", "xyz.inphinity.sun.security.mscapi.CRSACipher",
                           null, attrs));
                putService(new ProviderService(p, "Cipher",
                           "RSA/ECB/PKCS1Padding", "xyz.inphinity.sun.security.mscapi.CRSACipher",
                           null, attrs));
                return null;
            }
        });
